...
1. 동기적/일대일
    ...
    * 클라이언트가 서버에 요청을 보낸 후, 응답이 올 때까지 대기(block)하는 방식이다. 예: REST API 호출 - 주문 생성 요청 후 결과를 받을 때까지 대기
        
    * 우리가 전화를 하는 것처럼 상대방이 전화를 받아야 상호간의 통신이 가능한 것에 비유할 수 있다.
        
2. 비동기적 요청/일대일
    
    * 클라이언트가 요청을 보낸 후, 응답을 기다리지 않고 다른 작업을 수행할 수 있다.
        
    * 예: 이메일 발송 - 메일 전송 요청 후 전송 완료를 기다리지 않고 다음 작업 진행
...
3. 동기적/일대다
    ...
    * 예: 여러 서비스에 동시에 요청하고 모든 응답을 기다리는 경우 (예: 상품 상세 페이지 로딩 시 재고, 가격, 리뷰 서비스에 동시 요청)
...
우선, 비동기적 방식부터 간단히 설명한다. **비동기적으로 통신할 때는 kafka 혹은 AMQP**(대표적으로 Rabbit MQ) 방식을 채택한다. 이는 메시지 큐 아키텍처, 혹은 메시지 대기열 시스템으로, 나중에 이어질 아티클에서 자세히 설명하도록 한다.
...
하지만, 이런 파일이 서비스간 통신에 매우 잦은 빈도로 사용되다 보면, 로그에 쌓이는 데이터도 많을 뿐만 아니라 네트워크 대역폭과 저장 공간을 더 많이 사용하게 되어 비용이 증가한다. 사람이 읽기 쉽게 하기 위해 더 많은 텍스트를 사용하기 때문이다.

그래서 이를 해결하기 위해 바이너리 방식의 Protocol Buffers, Avro 등을 사용하기도 한다.

이들은 IDL(Interface Definition Language)을 사용하여 어떤 함수를 제공할지, 어떤 데이터를 주고받을지 미리 정의한다. 예를 들어, 한쪽에서는 Python, 다른 한쪽에서는 Java를 쓴다고 할 때, IDL로 중립적인 형식으로 정의해두면 각 언어에 맞는 코드를 자동 생성할 수 있다.

실제 데이터 전송 시에는 바이너리 형태로 변환된다:

> Python의 User 객체 → 바이너리로 직렬화 → 네트워크 전송 → Java의 User 객체로 역직렬화
